...
const UserByRole: NextPage = () => {
  ...
  const pageTitle =
    role.toString().charAt(0).toUpperCase() + role.toString().slice(1);
  const baseEndpoint: any = "user?user_role=" + role;
  ...
  useEffect(() => {
    fetchUser(0); // fetch page 1 of users
  }, [role]);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (tableProps: any) => (
        <>
          <Link href={"/e-auction-admin/help-category/edit/" + tableProps._id}>
          ...
          {/* <Link href={"/e-auction-admin/help-category/view/" + tableProps._id}>
