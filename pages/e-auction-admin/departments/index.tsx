...
const DepartmentList: NextPage = () => {
  ...
  const fetchUsers = async (page: any) => {
    ...
    try {
      const response = await fetch(
        `${mainApiUrl}common/view/department?page=${page}&per_page=${perPage}&delay=1`
      );
  ...
  const handlePerRowsChange = async (newPerPage: any, page: any) => {
    ...
    try {
      const response = await fetch(
        `${mainApiUrl}common/view/department?page=${page}&per_page=${newPerPage}&delay=1`
      );
      const respoData = await response.json();
      setData(respoData.records);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (tableProps: any) => (
        <>
          <Link href={"/e-auction-admin/departments/edit/" + tableProps._id}>
          ...
          <Link href={"/e-auction-admin/departments/view/" + tableProps._id}>
      ...
      <div className="p-6 pt-0">
        <div className="row">
          ...
          <div className="col-md-4 mt-3 text-right">
            <Link href={"/e-auction-admin/departments/add"}>
