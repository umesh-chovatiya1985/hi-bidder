...
const AddHelpList: NextPage = () => {
  ...
  const uploadImage = async () => {
    ...
    formData.append("table", "help-list");
  ...
  const submitEventHandler = async (e: any) => {
    e.preventDefault();
    if (!sourceFile) {
      toast.error("Please, Select photo for help.", {
        position: "top-right",
        autoClose: 5000,
        closeOnClick: true,
        theme: "dark",
  ...
  const fetchHelpTopic = async () => {
    ...
    if (categoryData.ok) {
      setHelpTopic(categoryJson.records);
    }
  };
  const fetchCategories = async (topicId: any) => {
    const categoryData = await fetch(
      mainApiUrl + "helps/help-category/help-topic/" + topicId
    );
    const categoryJson = await categoryData.json();
    if (categoryData.ok) {
                          ...
                          >
                            ...
                            {categories &&
                              (categories.length > 0 ? (
                                ...
                                <option value="">No Help Category</option>
                              ))}
                          </select>
                        </div>
                        <div className="relative w-full mb-3">
