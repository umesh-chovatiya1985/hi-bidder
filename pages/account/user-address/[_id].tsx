...
const BuyerAddress: NextPage = () => {
    ...
    const onSubmit = async (data: buyeraddress) => {
        try {
            const addressRespo = await fetch(mainApiUrl+"user/user-address/"+primaryId, {
                method: "PUT",
        ...
        try {
            ...
            if(addressRespo.ok){
                if(addressJson.status == 1){
                    // console.log(addressJson.record['address_one']);
                    formCreation.map((field: any) => (
                        setValue(field.formName, addressJson.record[field.formName])
                    ));
                }
            }
