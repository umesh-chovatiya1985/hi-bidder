    ...
    const handleCase: ResponseFuncs = {
        GET: async (req: NextApiRequest, res: NextApiResponse) => {
            ...
            await ProductAuction.findById(id).then((resp: any) => {
                res.json({ status: (resp ? 1 : 0), record: resp });
        ...
        DELETE: async (req: NextApiRequest, res: NextApiResponse) => {
            const { ProductAuction } = await productauctionSchema();
            const delRecord = {isDeleted: true, deletedAt: new Date(), deletedBy: token.sub};
            await ProductAuction.findByIdAndUpdate(id, delRecord, { new: true }).then((resp: any) => {
